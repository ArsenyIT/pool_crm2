...
| Статус        | Цвет                  |
|---------------|-----------------------|
| Новая заявка  | Синий('bg-primary')   |
| Одобрена      | Зелёный('bg-success') |
| Отклонена     | Красный('bg-danger')  |
| В обработке   | Жёлтый('bg-warning')  |
...
- Волны из простых фигур
- Движение влево с опусканием/поднятием
- Дельфины: плавают, выпрыгивают, ныряют с брызгами
- Кит: Подплывает справа, открывает рот, выдыхает облако с текстом

### При наведении
- Карточки поднимаются на 5px
- Иконки увеличиваются
- Кнопки меняют цвет и поднимаются
- Всплывающие картинки ('kids.png', 'pool.png')

### Галерея
- Фото появляется размытым → становится чётким
- Управление: колёсико мыши, стрелки, клавиши ← →, ESC
- Карусель миниатюр с прокруткой

### Краб в Футере
- Бегает по кругу
- При клике ускоряется
- Выпускает пузырьки
...
| Светло-синий (фон) | '#e0f2fe' →  '#7dd3fc' |
| Тёмный текст       | '#e0f172a'             |
| Светлый текст      | '#475569'              |
| Успех              | '#10b981'              |
| Ошибка             | '#ef4444'              |
